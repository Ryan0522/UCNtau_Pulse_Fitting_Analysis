...
PDF_Lookup::PDF_Lookup(double coarse_bw, double fine_bw)
    : coarseBinWidth_(coarse_bw), fineBinWidth_(fine_bw)
{
    if (!csv.path.empty()) {
...
bool PDF_Lookup::load_csv(const std::string& path) {
    ...
    for (string h; std::getline(hs, h, ',');) headers.push_back(h);
    if (headers.empty() || headers[0] != "Time(us)") return false;

    const size_t C = headers.size();
    if (C < 2) return false;

    vector<vector<double>> cols(C);
    ...
    for (size_t c = 1; c < C; ++c) {
        ...
        normalize_series(p001, dt001);

        BasePDF b;
        b.binWidth = 1.0;
        b.fineBinWidth = 0.25;
        downsample(p001, 0.01, b.p_f, b.fineBinWidth);
...
static int PDF_Lookup::parse_segment_name(const std::string& header) {
    // Accept "Segment_12"
...
static void PDF_Lookup::normalize_series(std::vector<double>& v, double dt) {
...
static void PDF_Lookup::downsample(const std::vector<double>& src, double dt_src, std::vector<double>& dst, double dt_dst) {
    if (~approx_int_multiple(dt_dst, dt_src))
        throw std::runtime_error("downsample: dt_dst must be a integer multiple of dt_src.");
    ...
    for (int i = 0; i < N; ++i) {
        double s = 0.0;
        int a = i * R;
        for (int k = 0; k < R; ++k) s += src[a + k];
        dst[i] = s * (dt_src / dt_dst);
    }
}

static bool PDF_Lookup::approx_int_multiple(double value, double step, double rel_eps = 1e-9) {
    double q = value / step;
